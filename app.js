...
var posts = [{
    title: 'Default Post',
    body: 'Just a ranod0m default post'
},];
...
app.get('/post/:postTitle', (req, res)=> {
    if(post = contains(posts, req.params.postTitle)) {
...
function contains(posts, title) {
    for (let post of posts) {
        if(_.lowerCase(post.title) === _.lowerCase(title)) return post;
    }
    return null;
}